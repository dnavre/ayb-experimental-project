...
if(isset($_GET['id'])) {
    $stmt = $db->prepare("SELECT s.id, s.name, s.price, s.featured, p.src photo_src FROM souvenir s ORDER BY s.name DESC left join photo p on s.main_photo_id = p.id WHERE s.visible=1 and s.category_id=" . $_GET['id'] . " ORDER BY s.create_date DESC");
    $stmt->execute();
